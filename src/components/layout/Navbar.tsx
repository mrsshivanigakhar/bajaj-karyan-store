        ...
        <div className="flex items-center gap-2">
          ...
          <span className="truncate">
            {settings.delivery_info ||
              `Local Grocery & Confectionery in ${settings.city || "Firozpur"}`}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between gap-3 sm:gap-6">
          ...
          <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
            ...
            >
              <Heart
                className={`w-4 h-4 transition-transform ${
                  totalFavoritesCount > 0
                    ? "fill-pink-400 text-pink-400 animate-heartbeat-slow"
                    : "text-pink-300"
                }`}
              />
              ...
              {totalFavoritesCount > 0 && (
                <span
                  key={`fav-${totalFavoritesCount}`}
                  className="bg-[#ff4d6d] text-white font-bold text-[10px] px-1.5 py-0.5 rounded-full min-w-[18px] text-center shadow-xs transition-transform animate-heartbeat-slow"
                >
                  {totalFavoritesCount}
                </span>
              )}
            ...
            >
              ...
              {totalItemsCount > 0 && (
                <span
                  key={`list-${totalItemsCount}`}
                  className="bg-white text-[#590d22] font-bold text-xs px-1.5 py-0.5 rounded-full min-w-[20px] text-center shadow-xs transition-transform animate-heartbeat-fast"
                >
                  {totalItemsCount}
                </span>
              )}
